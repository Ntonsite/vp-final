...
<div class="container">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form method="post" action="<?php echo site_url('engine/create') ?>" enctype="multipart/form-data">
						...
						<a href="<?php echo site_url('engine') ?>">
	...
	<input type="hidden" id="addUser_success" value="<?php echo $this->session->flashdata('user_add'); ?>">
		...
		<?php if (!empty($result)) {
			foreach ($result as $row) { ?>
				<tr>
					...
					<th scope="row"><?php if ($row->role == 1) {
							echo "engine";
						} else {
							echo "Member";
						} ?></th>
					<th scope="row"><?php if ($row->is_active == 1) {
							echo "Active";
						} else {
							echo "Not Active";
						} ?></th>
					<td><a class="btn btn-sm btn-warning"
						   href="<?php echo site_url('engine/edit'); ?>/<?php echo $row->user_id; ?>">Edit</a>
						<a onClick="javascript: return confirm('Please confirm deletion');"
						   class="btn btn-sm btn-danger"
						   href="<?php echo site_url('engine/delete'); ?>/<?php echo $row->user_id; ?>">Delete</a></td>
				</tr>
			<?php }
		} ?>
		</tbody>
