		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form method="post" action="<?php echo site_url('Contract/create') ?>"
						  enctype="multipart/form-data">
						<div class="form-group">
							<label for="exampleInputEmail1">Contract Name</label>
							<input id="contractName" required="" type="text" class="form-control" name="contract_name"
								   aria-describedby="emailHelp" placeholder="Contract Name">
						</div>
						<div class="form-group">
							<label for="exampleInputEmail1">Description</label>
							<textarea class="form-control" placeholder="contract description" name="description" id=""
							...
							<select name="vendor" class="form-control">
								<?php if (!empty($vendor)) {
									foreach ($vendor as $vendors) {
										?>
										<option
											value="<?php echo $vendors->vendorID; ?>"><?php echo $vendors->v_name; ?></option>
									<?php }
								} ?>
							</select>
						</div>
						<div class="form-group">
							...
							<input required="" type="date" class="form-control" name="date_of_start"
								   aria-describedby="emailHelp" placeholder="Date">
						</div>
						<div class="form-group">
							<label for="exampleInputEmail1">Expiry Date</label>
							<input required="" type="date" class="form-control" name="date_of_expiry"
								   aria-describedby="emailHelp" placeholder="Date">
						</div>
						<div class="form-group">
							<label for="exampleInputEmail1">Attachment (PDF& DOC)</label>
							<input required="" accept="image/*,.pdf,.doc,.docx" type="file" class="form-control"
								   name="file" aria-describedby="emailHelp" placeholder="attach a picture">
						</div>
			...
			foreach ($result as $row) { ?>
				<tr>
					...
					<td><a class="btn btn-sm btn-primary"
						   href="<?php echo base_url('uploads/contracts'); ?>/<?php echo $row->file; ?>">View</a>
						<?php
						if ($group == 3 || $role==1) {
							?>
							<a class="btn btn-sm btn-warning"
							   href="<?php echo site_url('Contract/edit'); ?>/<?php echo $row->id; ?>">Edit</a>
							<a onClick="javascript: return confirm('Please confirm deletion');"
							   class="btn btn-sm btn-danger"
							   href="<?php echo site_url('Contract/delete'); ?>/<?php echo $row->id; ?>">Delete</a>
							<?php
						}
						?>
					</td>
